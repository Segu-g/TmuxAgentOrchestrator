...
4. [test_agents.py — エージェント動作（1テスト）](#4-test_agentspy--エージェント動作1テスト)
5. [test_bus.py — メッセージバス（6テスト）](#5-test_buspy--メッセージバス6テスト)
6. [test_messaging.py — メールボックス（10テスト）](#6-test_messagingpy--メールボックス10テスト)
7. [test_orchestrator.py — タスク配信・P2P・タイムアウト（11テスト）](#7-test_orchestratorpy--タスク配信p2pタイムアウト11テスト)
8. [test_tmux_interface.py — tmux ラッパー（7テスト）](#8-test_tmux_interfacepy--tmux-ラッパー7テスト)
9. [test_worktree.py — Git Worktree マネージャー（9テスト）](#9-test_worktreepy--git-worktree-マネージャー9テスト)
10. [テストインフラストラクチャ](#10-テストインフラストラクチャ)
11. [カバレッジとギャップ](#11-カバレッジとギャップ)
12. [新しいテストの追加方法](#12-新しいテストの追加方法)

---

## 1. 概要

スイートには 6 ファイルにわたる **43 のテスト**があります。コアライブラリモジュールのすべての公開動作がカバーされています。実際の外部プロセス（tmux サーバー、`claude` CLI）との統合は完全に回避されています — モックまたは一時的な実 git リポジトリに置き換えられています。

```
tests/
├── test_agents.py          1テスト  — エージェントレベルの動作（コンテキストファイル）
...
├── test_tmux_interface.py  7テスト  — libtmux ラッパー
...
`ClaudeCodeAgent` や `CustomAgent` の代わりに使用する最小限の `Agent` サブクラス。
...
## 4. `test_agents.py` — エージェント動作（1テスト）

ライブの tmux セッションを必要としないエージェントレベルの動作をテストします。

### `test_custom_agent_writes_context_file`

**テスト内容:** `CustomAgent` が `start()` 中に `cwd_override` が提供されると `__orchestrator_context__.json` を作業ディレクトリに書き込むこと。

**セットアップ:** `CustomAgent` を `cwd_override=tmp_path` と `command="cat"`（stdin を読み取り出力を生成しない no-op サブプロセス）で起動します。`start()` 後にコンテキストファイルを読み取ってパースします。

**アサーション:**
- `__orchestrator_context__.json` が `tmp_path` に存在する。
- `ctx["agent_id"] == "ctx-test"`。
- `ctx["worktree_path"] == str(tmp_path)`。

**重要な理由:** REST API やメールボックスにコールバックする必要があるカスタムエージェント（メッセージの読み取りやサブエージェントの生成など）は、このファイルに依存してオーケストレーターの座標を検出します。このテストは、エージェントタイプに関わらずファイルが一貫して書き込まれること、および `cwd_override` パスが正しく記録されることを保証します。

---

## 5. `test_bus.py` — メッセージバス（6テスト）
...
## 6. `test_messaging.py` — メールボックス（10テスト）
...
## 7. `test_orchestrator.py` — タスク配信・P2P・タイムアウト（11テスト）
...
## 8. `test_tmux_interface.py` — tmux ラッパー（7テスト）
...
**シナリオ:** 指定された名前のセッションがすでに存在する。

**モックセットアップ:** `mock_server.find_where.return_value = existing_mock`。

**アサーション:** `new_session` は*呼ばれない*；既存のセッションが返される。
...
## 9. `test_worktree.py` — Git Worktree マネージャー（9テスト）
...
| `tmux_interface.py` | セッション作成/アタッチ、監視/監視解除、send_keys、キャプチャ、ハッシュ |
| `worktree.py` | セットアップ、削除、isolate=false、.gitignore、git 外、重複セットアップ |
| `agents/base.py` | `_write_context_file`（test_agents 経由）、タイムアウト強制、ステータスイベント |
| `agents/custom.py` | 起動時のコンテキストファイル書き込み（test_agents） |
...
| `CustomAgent` サブプロセス I/O | ライブのサブプロセスが必要；統合テストの領域 |
