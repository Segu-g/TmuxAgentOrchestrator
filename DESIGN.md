...
| **高** | **`__orchestrator_context__.json` からの API キー削除 + 推奨方式への移行** — v0.34.0 で導入されたセキュリティリスク。フェーズ 1: キーを独立ファイル `__orchestrator_api_key__` (`chmod 600`) に分離。フェーズ 2: libtmux `session.set_environment()` で環境変数注入に移行。詳細は §3「API キー配送のセキュリティ方針」参照。 |
...
### リサーチエージェント調査結果 (2026-03-05)

> 5テーマ (multi-LLM orchestration / clean architecture / 形式手法 / コンテキストエンジニアリング / TDD for agents) の横断調査。
> 既実装の重複を除いて新規バックログ候補のみを列挙する。

| 優先度 | 課題 | 根拠 |
|--------|------|------|
| 高 | **OpenTelemetry GenAI Semantic Conventions 準拠トレース出力** — `gen_ai.*` 属性 (token counts, tool calls, agent spans) を既存 `trace_id` ベースの構造化ログに付加し、Datadog/Jaeger/OTLP エクスポーターへ送信できるようにする | OpenTelemetry "AI Agent Observability" (2025) が業界標準に収斂しつつあり、Datadog も GenAI Semantic Conventions にネイティブ対応済み。現状の `trace_id` は相関のみでスパン階層がない。[opentelemetry.io/blog/2025/ai-agent-observability](https://opentelemetry.io/blog/2025/ai-agent-observability/) |
| 高 | **エージェントドリフト検出 (Agent Stability Index)** — 同一エージェントの連続出力に対し role adherence スコアを算出し、一定閾値を下回ったら `agent_drift_warning` STATUS イベントを発行する `DriftMonitor` を実装する | arXiv:2601.04170 "Agent Drift: Quantifying Behavioral Degradation" (2025) が12次元の Agent Stability Index (ASI) を提案。現行の ContextMonitor はトークン量のみ監視しており、役割逸脱・タスク重複を検出できない。[arxiv.org/abs/2601.04170](https://arxiv.org/abs/2601.04170) |
| 高 | **チェックポイント永続化による中断再開** — `ResultStore` (v0.24.0 JSONL) を拡張し、タスク進行状態・エージェント変数スナップショットを SQLite に保存。プロセス再起動後に `--resume` フラグで最後のチェックポイントから継続できるようにする | LangGraph は checkpointer + PostgresSaver で fault-tolerant persistence を提供 (LangChain docs 2025)。TmuxAgentOrchestrator はプロセス再起動でキュー・ワークフロー状態が消える。[docs.langchain.com/oss/python/langgraph/persistence](https://docs.langchain.com/oss/python/langgraph/persistence) |
| 高 | **`ProcessPort` 抽象インターフェースの抽出** — `ClaudeCodeAgent` が直接 `libtmux.Pane` に依存している箇所を `ProcessPort` protocol に置き換え、`TmuxProcessAdapter` と `StdioProcessAdapter` (テスト用) を実装する。`ClaudeCodeAgent` がこのポートのみに依存するよう依存方向を逆転させる | Martin "Clean Architecture" (2017) Ch.22 のポート&アダプターパターン。現状は `agents/claude_code.py` が `libtmux` を直接 import しており、tmux なし環境でのユニットテストが不可能。Naoyuki Sakai "AI Agent Architecture: Mapping Domain, Agent, and Orchestration to Clean Architecture" (2025) が同パターンを AI エージェントに適用。[medium.com/@naoyuki.sakai/ai-agent-architecture...](https://medium.com/@naoyuki.sakai/ai-agent-architecture-mapping-domain-agent-and-orchestration-to-clean-architecture-fd359de8fa9b) |
| 中 | **`UseCaseInteractor` 層の抽出** — `web/app.py` の FastAPI ハンドラーが `orchestrator.*` メソッドを直接呼ぶ箇所を `SubmitTaskUseCase`, `CancelTaskUseCase` 等の Use Case クラスに置き換え、Web 層とドメイン層の依存方向を逆転させる | Martin "Clean Architecture" §22: Use Case Interactor がアプリケーション固有ビジネスルールを保持し、Web/CLI/TUI のどのインターフェースからも同一ロジックを呼べる。現状は FastAPI ハンドラーにロジックが漏れており、TUI から同機能を使うときに重複する。[blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html) |
| 中 | **P2P 許可テーブルの TLA+ 形式仕様化** — `_is_hierarchy_permitted()` と `p2p_permissions` の状態遷移を TLA+ で記述し、TLC model checker で「メッセージがルーティングループに入らない」「P2P 許可のない経路へは届かない」を全状態空間で検証する | Lamport "Specifying and Verifying Systems with TLA+" (2024) および AWS による TLA+ 活用実績。現行の `_forwarded` フラグでループを防いでいるが、将来の変更で迂回経路が生まれるリスクを property 検証で事前に検出できる。[lamport.azurewebsites.net/pubs/spec-and-verifying.pdf](https://lamport.azurewebsites.net/pubs/spec-and-verifying.pdf) |
| 中 | **エージェント状態機械の Hypothesis ステートフルテスト拡張** — `AgentStatus` の遷移 (IDLE→BUSY→IDLE/ERROR/DRAINING) を `RuleBasedStateMachine` (Hypothesis) でモデル化し、任意の割り込み・タイムアウト・リカバリシーケンスを自動生成してデッドロック・不変量違反を検証する | Hypothesis `stateful` モジュール (QuickCheck ICFP 2000 由来) は任意のシーケンスを生成して不変量検証を行う。v0.4.0 で導入した PBT はステートレスなプロパティテストのみで、状態遷移シーケンスのテストは未カバー。[hypothesis.readthedocs.io/en/latest/stateful.html](https://hypothesis.readthedocs.io/en/latest/stateful.html) |
| 中 | **MIRIX 型エピソード記憶ストア** — 各エージェントの `NOTES.md` に加えて、タスク完了ごとに `{task_id, summary, outcome, lessons}` を軽量 JSONL エピソードログとして蓄積し、次タスク開始時に直近N件を system prompt に付加するエピソード記憶を実装する | arXiv:2507.07957 "MIRIX: Multi-Agent Memory System" (2025) が RAG ベースラインより 35% 精度向上を達成。現行の `/summarize` → `NOTES.md` は単一ファイルに上書きされ過去エピソードが失われる。[arxiv.org/pdf/2507.07957](https://arxiv.org/pdf/2507.07957) |
| 中 | **スライディングウィンドウ + 重要度スコアによるコンテキスト圧縮** — `ContextMonitor` が 75% 閾値を検出したとき、単純に `/summarize` を注入するのではなく、タスクプロンプトとの TF-IDF 類似度で行の重要度スコアを算出し、スコア下位 40% を削除したうえで圧縮済みコンテキストを注入する | Liu et al. "Lost in the Middle" TACL 2024 が中央部情報の忘却を実証。JetBrains Research "Cutting Through the Noise" (2025-12) が重要度スコアリングによる大幅なコスト削減を実証。現行の `/summarize` はプロンプトとの関連度を考慮せず一律圧縮する。[blog.jetbrains.com/research/2025/12/efficient-context-management](https://blog.jetbrains.com/research/2025/12/efficient-context-management/) |
| 中 | **Evaluation-Driven Development (EDDOps) サポート** — `/tdd` コマンドに LLM-as-judge 評価ステップを追加し、生成コードを `pytest` 実行後に別エージェント (judge) が正確性・保守性・セキュリティの3軸スコアを採点。スコアが閾値未満なら自動的に RED フェーズに差し戻す | arXiv:2411.13768 "Evaluation-Driven Development and Operations of LLM Agents" (2025) が EDDOps プロセスモデルと参照アーキテクチャを提案。現行の `/tdd` はチェックリスト表示のみで客観的品質評価がない。[arxiv.org/html/2411.13768v3](https://arxiv.org/html/2411.13768v3) |
| 低 | **`AgentRegistry` の独立モジュール化** — `Orchestrator` に集中している `_agents`, `_agent_parents`, `_draining_agents` 等のレジストリ操作を `AgentRegistry` クラスとして抽出し、Orchestrator はこれを依存注入で受け取る | §10.5 で既提案。Orchestrator が God Object 化しており、1400行超の単一クラスになっている。DDD Aggregate 原則および LangGraph のノード/グラフ分離設計が参考。[medium.com/autogen-framework-multi-agents](https://galileo.ai/blog/autogen-framework-multi-agents) |
| 低 | **構造化デバッグ: 任意シーケンスのトレースリプレイ** — LangGraph の "time travel" デバッグ機能に相当する機能として、`ResultStore` の JSONL + bus イベントログを組み合わせて過去の実行シーケンスを再現し、どのステップで失敗したかを特定できる CLI コマンド `tmux-orchestrator replay` を追加する | LangGraph + LangSmith の replay 機能は業界標準デバッグ手法として認識 (LangChain docs 2025)。Galileo AI の "Why Multi-Agent LLM Systems Fail" (2025) が「非決定的挙動のリプレイ不可」を主要な問題として挙げている。[galileo.ai/blog/multi-agent-llm-systems-fail](https://galileo.ai/blog/multi-agent-llm-systems-fail) |
| 低 | **LLM-as-Judge による並列エージェント出力の自動スコアリング** — `POST /tasks/batch` で投入した Best-of-N タスクの RESULT を自動収集し、別の ClaudeCodeAgent (judge role) が各出力を多次元スコアリングして最優秀を選択・通知する `BestOfNEvaluator` を実装する | Cemri et al. arXiv:2503.13657 "Why Do Multi-Agent LLM Systems Fail?" (2025) が検証ギャップ (21.30%) を主要失敗要因として特定。現行の v0.15.0 AHC デモはユーザーが手動でスコアを確認する設計。[arxiv.org/pdf/2503.13657](https://arxiv.org/pdf/2503.13657) |

---

### 開発手法ワークフロー候補 (リサーチエージェント調査, 2026-03-05)

> 調査テーマ: TDD マルチエージェントワークフロー / クリーンアーキテクチャ分解 / コンテキストエンジニアリング / 形式手法 / コンテキストエンジニアリング(日本語文献)。
> 視点: フレームワーク内部のリファクタリングではなく、**ユーザーがこのフレームワークを使って各開発手法を実践するための機能・ワークフローテンプレート**に絞って列挙する。

| 優先度 | 課題 | 根拠・出典 |
|--------|------|-----------|
| 高 | **TDD ワークフローテンプレート (`type: tdd`) — Red→Green→Refactor を3エージェントに自動分担する `POST /workflows/tdd` エンドポイントを追加** — `test-writer` (failing test のみを書く)・`implementer` (テストを見て最小実装を書く)・`refactorer` (実装を見て改善する) の3ロールを YAML で宣言し、各エージェントが**別コンテキストウィンドウ**で動作するワークフロー DAG を一発投入できるようにする。コンテキスト分離が真の test-first を保証するため、単一エージェントの `/tdd` では実現不可能な品質が得られる | alexop.dev "Forcing Claude Code to TDD" (2025): 「1つのコンテキストウィンドウでは実装者の思考がテストに漏れる — 真の TDD には別コンテキストが必須」。TDFlow (arXiv:2510.23761, CMU/UCSD/JHU 2025): test-driven agentic workflow が SWE-Bench Lite で 88.8% pass rate (次点比 +27.8%)。[arxiv.org/abs/2510.23761](https://arxiv.org/abs/2510.23761) / [alexop.dev/posts/custom-tdd-workflow-claude-code-vue](https://alexop.dev/posts/custom-tdd-workflow-claude-code-vue/) |
| 高 | **クリーンアーキテクチャ分解ワークフロー — Director が機能要求を 4 レイヤー (domain / use-case / adapter / framework) に分解し、各レイヤーを専用ワーカーに割り当てる `POST /workflows/clean-arch` を追加** — Director タスクのプロンプトに「レイヤー境界ルール (依存方向・ポート定義) を PLAN.md に書いてから各ワーカーに委任せよ」と指示するワークフローテンプレートと、`.claude/prompts/roles/` に `domain-agent.md` / `usecase-agent.md` / `adapter-agent.md` / `framework-agent.md` の4ロールプロンプトファイルを提供する | AgentMesh (arXiv:2507.19902, 2025): Planner→Coder→Debugger→Reviewer の4ロール分担がソフトウェア開発タスクを自動化。Muthu (2025-11) "The Architecture is the Prompt": 「ヘキサゴナルアーキテクチャの境界がそのまま AI エージェントのコンテキスト制約になる」。[arxiv.org/abs/2507.19902](https://arxiv.org/abs/2507.19902) / [notes.muthu.co/2025/11/the-architecture-is-the-prompt-guiding-ai-with-hexagonal-design](https://notes.muthu.co/2025/11/the-architecture-is-the-prompt-guiding-ai-with-hexagonal-design/) |
| 高 | **役割別 system_prompt テンプレートライブラリ — `.claude/prompts/roles/` に tester / implementer / reviewer / spec-writer / domain-expert / judge の6種類のプロンプトファイルを提供し、YAML `system_prompt_file:` フィールドで参照できるようにする** — 各テンプレートには「あなたの役割・禁止事項・完了条件・/plan と /tdd の使い方」を標準化して記述し、CLAUDE.md の "Orchestrated Agent" セクションにベストプラクティスを追記する | Anthropic "Effective Context Engineering for AI Agents" (2025-09-29): 「コンテキストエンジニアリングとはプロンプト設計を超えた、推論時の情報エコシステム全体の管理」。Vellum "Best practices for building multi-agent systems" (2025): 役割特化プロンプトとステート分離が精度向上に最も効果的。[anthropic.com/engineering/effective-context-engineering-for-ai-agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) / [vellum.ai/blog/multi-agent-systems-building-with-context-engineering](https://www.vellum.ai/blog/multi-agent-systems-building-with-context-engineering) |
| 高 | **Codified Context インフラ — プロジェクト規約・アーキテクチャ決定・禁止事項を機械可読 YAML/JSON 仕様ファイルとして `.claude/specs/` に配置し、`context_files` で各エージェントに自動配布する仕組みを提供する** — `AgentConfig.context_spec_files` (glob パターン) を追加し、タスク開始時に仕様ファイルをワークツリーにコピーすることで、エージェントがセッションをまたいでもプロジェクト規約を忘れない Codified Context 基盤を実現する | Vasilopoulos arXiv:2602.20478 "Codified Context: Infrastructure for AI Agents in a Complex Codebase" (2026-02): 108,000行 C# 分散システムで 283 セッションにわたり規約を維持。Hot-memory constitution + Cold-memory spec documents の2層構造が有効。[arxiv.org/abs/2602.20478](https://arxiv.org/abs/2602.20478) |
| 中 | **DDD Bounded Context ワークフローテンプレート — Director が機能要求をドメインイベント・集約・コマンドに分解した EventStorming マップを PLAN.md に書き出し、境界コンテキストごとに専用ワーカー (各自が ubiquitous language 定義を context_files として受け取る) に実装を委任する `POST /workflows/ddd` を追加する** | Russ Miles "Domain-Driven Agent Design" (Engineering Agents, 2025): DDD の Bounded Context がマルチエージェントの責務分割境界に直接対応する。Bakthavachalu "Revolutionizing Enterprise AI: Applying DDD for Agentic Applications" (2025): 大手投資銀行の3 Bounded Context 実装事例 (Risk/Regulatory/Validation)。[engineeringagents.substack.com/p/domain-driven-agent-design](https://engineeringagents.substack.com/p/domain-driven-agent-design) |
| 中 | **形式仕様エージェントステップ — `POST /workflows` の DAG に `type: spec` タスクを追加可能にし、spec-writer エージェントが Alloy または TLA+ の軽量仕様を書いてから impl エージェントがコードを書く「仕様→実装」ハンドオフパターンを公式サポートする** — ハンドオフは `reply_to` + PLAN.md 経由で実現し、`.claude/prompts/roles/spec-writer.md` テンプレートに「Alloy シグネチャ / TLA+ 不変量の書き方と impl エージェントへの渡し方」を記述する | Benjamin Congdon "The Coming Need for Formal Specification" (2025-12): 「AI 生成コードが増えるほど、それが何をすべきかの仕様が重要になる」。Hou et al. "Position: Trustworthy AI Agents Require Formal Methods" (2025): TLA+/Hoare 表明の LLM エージェントへの統合を提言。SYSMOBENCH (arXiv:2509.23130, 2025): LLM の TLA+ 仕様生成能力を 200 システムモデルで評価。[benjamincongdon.me/blog/2025/12/12/the-coming-need-for-formal-specification](https://benjamincongdon.me/blog/2025/12/12/The-Coming-Need-for-Formal-Specification/) / [arxiv.org/pdf/2509.23130](https://arxiv.org/pdf/2509.23130) |
| 中 | **コンテキスト4戦略ガイドを CLAUDE.md と `.claude/prompts/` に体系化 — 「書き込み (NOTES.md/PLAN.md)・選択 (context_files)・圧縮 (/summarize)・分離 (worktree + 別コンテキスト)」の4戦略を役割ごとに組み合わせたベストプラクティスチートシートを提供し、CLAUDE.md の "Running as an Orchestrated Agent" セクションに追記する** | Algomatic Tech Blog "AIエージェントを支える技術: コンテキストエンジニアリングの現在地" (2025-10): 書き込み・選択・圧縮・分離の4戦略フレームワークが日本語コミュニティで広まりつつある。技術評論社 "LLMの原理、RAG・エージェント開発から読み解くコンテキストエンジニアリング" (2026): 刊行予定の体系書。[tech.algomatic.jp/entry/2025/10/15/172110](https://tech.algomatic.jp/entry/2025/10/15/172110) / [gihyo.jp/book/2026/978-4-297-15419-6](https://gihyo.jp/book/2026/978-4-297-15419-6) |
| 中 | **PairCoder ワークフローテンプレート — Navigator (計画・高レベル戦略) と Driver (実装・テスト・デバッグ) の2エージェントがペアプログラミングを模倣する `POST /workflows/pair` を追加** — Navigator は PLAN.md とフィードバックを生成し Driver に送信 (`reply_to`)、Driver が実装して結果を返すサイクルを Workflow DAG の `depends_on` チェーンで表現する。TDD ワークフローとの組み合わせで Driver に `/tdd` を自動実行させることも可能 | FlowHunt "TDD with AI Agents" (2025): 「PairCoder が Navigator/Driver の役割分担でペアプログラミングを再現し、単一エージェント比でコード品質が向上」。Tweag "Agentic Coding Handbook - TDD" (2025): コンテキスト分離によるペアプログラミング相当のアプローチを推奨。[flowhunt.io/blog/test-driven-development-with-ai-agents](https://www.flowhunt.io/blog/test-driven-development-with-ai-agents/) / [tweag.github.io/agentic-coding-handbook/WORKFLOW_TDD](https://tweag.github.io/agentic-coding-handbook/WORKFLOW_TDD/) |
| 中 | **AgentMesh 型 4ロール開発パイプラインデモ (`Planner → Coder → Debugger → Reviewer`) — `examples/agentmesh_config.yaml` と `POST /workflows` 呼び出しスクリプトを提供し、1つの機能要求から完全実装+レビューまでを4エージェントが自動処理するデモシナリオを追加する** | AgentMesh (arXiv:2507.19902, 2025): Planner が要求→サブタスク分解 → Coder が実装 → Debugger がテスト・修正 → Reviewer が最終評価、のパイプラインが実証済み。TmuxAgentOrchestrator の Workflow DAG + tags (v0.18.0) + target_group (v0.31.0) で同パターンを宣言的に記述できる。[arxiv.org/abs/2507.19902](https://arxiv.org/abs/2507.19902) |
| 低 | **`/spec` スラッシュコマンドの追加 — オーケストレートされたエージェントが Alloy/TLA+ 軽量仕様を `SPEC.md` に書き出し、親エージェントに `spec_created` STATUS イベントを通知する `/spec <invariant description>` コマンドを `.claude/commands/spec.md` として提供する** — `/plan` → `/spec` → `/tdd` のフローを公式サポートし、仕様を外部記憶として残すことでコンテキストロット (context rot) を防ぐ | Anthropic "Effective Context Engineering" (2025-09): 「計画・仕様を外部ファイルに書くことがコンテキストロット対策の基本」。Hou et al. 形式手法ポジションペーパー (2025): LLM エージェントへの形式表明統合が信頼性向上に必須と主張。[anthropic.com/engineering/effective-context-engineering-for-ai-agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) |
| 低 | **ワークフローテンプレートライブラリ (`examples/workflows/`) に TDD / PairCoder / CleanArch / DDD / SpecFirst の5種類の参照 YAML を収録** — 各 YAML は `POST /workflows` で直接投入でき、エージェント数・ロール・`system_prompt_file` 参照・`context_files`・`required_tags` を含む自己完結した例として機能する。README に各テンプレートのユースケースと期待出力を記載する | CrewAI の YAML-driven workflow approach (2025) が「ドキュメントとしての設定ファイル」を普及させた。TmuxAgentOrchestrator が同等の参照テンプレートを提供することで、ユーザーが各手法を試す敷居を大幅に下げられる。[datacamp.com/tutorial/crewai-vs-langgraph-vs-autogen](https://www.datacamp.com/tutorial/crewai-vs-langgraph-vs-autogen) |

---

### 意思決定・議論ワークフロー候補 (リサーチエージェント調査, 2026-03-05)

> 調査テーマ: マルチエージェント討論・合意形成 / アーキテクチャレビュー支援 / ソクラテス的対話と構造化論証 / 意思決定自動化 (日本語文献含む) / 実世界ケーススタディ。
> コード生成以外の意思決定・設計議論・批評ワークフローを支援するため、TmuxAgentOrchestrator が提供できる具体的な機能・テンプレートを列挙する。

#### 背景: マルチエージェント討論の研究知見

- **Du et al. (ICML 2024) "Improving Factuality and Reasoning in Language Models through Multiagent Debate"** ([arxiv.org/abs/2305.14325](https://arxiv.org/abs/2305.14325)): 複数の LLM が互いの回答を批評し合うことで、事実性・推論精度が単一エージェント比で有意に向上 (数学推論・伝記事実で顕著)。議論が進むにつれ「全員が誤り → 正解に収束」するケースも確認されており、単純多数決を超えた改善が得られる。
- **Chan et al. (ICLR 2024) "ChatEval"** ([arxiv.org/abs/2308.07201](https://arxiv.org/abs/2308.07201)): 評価者 LLM を多数用意して討論させることで、単一 LLM 評価より人間判断との一致率が 2–6% 向上。役割の多様性 (異なるペルソナ) が本質的で、同一ロールの複数エージェントは性能を低下させる。
- **A-HMAD (Springer, 2025)** ([link.springer.com/article/10.1007/s44443-025-00353-3](https://link.springer.com/article/10.1007/s44443-025-00353-3)): 異種エージェント (論理推論 / 事実検証 / 戦略計画 の役割分担) による適応的討論が、均質エージェントより +6–10% の精度向上。伝記正答率: 単一 LLM 60% → MAD 74% → A-HMAD 80.6%。
- **失敗モード — PEACEMAKER OR TROUBLEMAKER (arXiv:2509.23055)**: 迎合的 (sycophantic) エージェントが早期収束を引き起こし、正解に達する前に議論が終わるリスク。CONSENSAGENT ([aclanthology.org/2025.findings-acl.1141](https://aclanthology.org/2025.findings-acl.1141)) がプロンプト動的リファインでこれを緩和。
- **最適ラウンド数**: 3 ラウンドで性能が飽和する傾向 (Chen et al. 2024)。それ以上は変動が大きくなり効果が薄れる。推論タスクには投票、知識タスクには合意形成が推奨 ([arxiv.org/abs/2502.19130](https://arxiv.org/abs/2502.19130))。
- **要件エンジニアリングへの応用**: MAD が要件分類 F1 スコアを 0.726 → 0.841 に改善 ([arxiv.org/abs/2507.05981](https://arxiv.org/abs/2507.05981))。
- **SocraSynth** ([arxiv.org/abs/2402.06634](https://arxiv.org/abs/2402.06634)): モデレーター + 2対立エージェント + LLM ジャッジ の構成で、論証の質を形式論理と Socratic 推論で評価するフレームワーク。対立→協調への段階的移行が特徴。
- **DEBATE フレームワーク (ACL 2024)** ([arxiv.org/abs/2405.09935](https://arxiv.org/abs/2405.09935)): Commander / Scorer / Critic (Devil's Advocate) の3エージェント構成で NLG 評価の SOTA を更新。悪魔の代理人 (Critic) が他エージェントの論拠を意図的に批判することで、単一 LLM 評価のバイアスを解消。
- **Delphi × LLM**: Real-Time AI Delphi (RT-AID, ScienceDirect 2025) が古典的デルファイ法を LLM で自動化する方法論を提案。専門家コメントをLLM生成テキストで代替し、複数ラウンドで意見を収斂させる ([arxiv.org/abs/2502.21092](https://arxiv.org/abs/2502.21092))。

| 優先度 | 課題 | 根拠・出典 |
|--------|------|-----------|
| 高 | **`POST /workflows/debate` — Advocate + Critic + Judge の3エージェント討論ワークフロー** — `advocate` エージェントが提案 (設計案・方針・ADR 草稿など) を作成し、`critic` エージェントが反論と改善点を提示、`judge` エージェントが議論を総合して最終判断を文書化する Workflow DAG を一発投入できるエンドポイントを追加する。ラウンド数は 2–3 回を上限とし (`max_rounds` パラメータ)、各エージェントは別コンテキストウィンドウで動作するため模倣なしの独立した評価が保証される。出力は `DECISION.md` (判断理由・代替案・トレードオフ) としてスクラッチパッドに書き出す | Du et al. ICML 2024: 議論ラウンドで事実性・推論精度が有意向上。A-HMAD (Springer 2025): 役割異種化で +10% 精度向上。DEBATE (ACL 2024 arXiv:2405.09935): Commander/Critic 3エージェント構成が NLG 評価 SOTA。[arxiv.org/abs/2305.14325](https://arxiv.org/abs/2305.14325) / [arxiv.org/abs/2405.09935](https://arxiv.org/abs/2405.09935) |
| 高 | **`.claude/prompts/roles/critic.md` — 批判的思考に特化したシステムプロンプトテンプレート** — 「あなたは提案に対して意図的に反論する Devil's Advocate である。欠陥・前提の誤り・見落とされたリスクを3点以上指摘し、対案を提示してから議論を終了せよ。他エージェントの意見に同調することを禁じる」と明示し、迎合 (sycophancy) を抑制するシステムプロンプトを提供する。同様に `advocate.md` (提案擁護)・`judge.md` (中立統合) の2ファイルもセットで提供し、`debate` ワークフローテンプレートから `system_prompt_file:` で参照できるようにする | ChatEval (ICLR 2024): 役割の多様性が討論の性能を決定づける — 同一ロールの複数エージェントは性能低下。CONSENSAGENT (ACL 2025): 迎合抑制プロンプトが正答率と効率の両方を改善。[arxiv.org/abs/2308.07201](https://arxiv.org/abs/2308.07201) / [aclanthology.org/2025.findings-acl.1141](https://aclanthology.org/2025.findings-acl.1141) |
| 高 | **`POST /workflows/adr` — Architecture Decision Record (ADR) 自動生成ワークフロー** — 技術的設計判断 (DB 選定・API 設計・モジュール境界など) を入力として `proposer` (案提示) → `reviewer` (技術的批評) → `synthesizer` (ADR 文書化) の3エージェントが `DECISION.md` を生成するワークフローテンプレートを提供する。出力は MADR フォーマット (title / status / context / decision / consequences) に準拠させ、`context_files` に既存 ADR を渡すことで過去の決定との整合性を保つ | MAD in Requirements Engineering (arXiv:2507.05981, 2025): MAD が要件分類 F1 を 0.726→0.841 に向上させ、設計判断の一貫性を改善。SocraSynth (arXiv:2402.06634): モデレーター + 対立エージェント + ジャッジ構成が設計討論に直接適用可能。[arxiv.org/abs/2507.05981](https://arxiv.org/abs/2507.05981) / [arxiv.org/abs/2402.06634](https://arxiv.org/abs/2402.06634) |
| 中 | **`POST /workflows/delphi` — 複数ラウンド合意形成 (Delphi 型) ワークフロー** — 3–5 名の専門家エージェント (各自が異なるペルソナ: セキュリティ / パフォーマンス / 保守性 / UX / コスト) が匿名で意見を提出し、モデレーターエージェントが中央値と分布を集計して次ラウンドにフィードバックする Delphi サイクルを最大 3 ラウンド実行する Workflow DAG を追加する。各ラウンドの出力を `delphi_round_{n}.md` に保存し、最終的な合意文書を `consensus.md` に書き出す | RT-AID "Real-Time AI Delphi" (ScienceDirect 2025) が LLM によるデルファイ法の自動化を提案。Du et al. ICML 2024: 「エージェントが全員誤りでも討論で正解に収束するケースが多数存在する」→ 複数ラウンドの価値を実証。[arxiv.org/abs/2502.21092](https://arxiv.org/abs/2502.21092) / [arxiv.org/abs/2305.14325](https://arxiv.org/abs/2305.14325) |
| 中 | **`POST /workflows/redblue` — Red Team / Blue Team 対抗評価ワークフロー** — `blue-team` エージェントが実装・設計案を作成し、`red-team` エージェントが攻撃者・懐疑論者の視点で脆弱性・欠陥・反証事例を列挙し、`arbiter` エージェントが両者の論拠を評価してリスク評価レポートを生成する。セキュリティレビュー・ビジネスケース検証・アーキテクチャ変更の影響分析などに利用できる汎用的な対抗評価テンプレートを提供する | Adversarial Multi-Agent Evaluation (OpenReview arXiv:2410.04663): 反復討論型評価がバイアス削減と判断精度向上に有効。Red-Teaming LLM MAS (ACL 2025, arXiv:2502.14847): エージェント間通信への攻撃を利用してシステムを評価する手法を提案。[arxiv.org/abs/2410.04663](https://arxiv.org/abs/2410.04663) / [arxiv.org/abs/2502.14847](https://arxiv.org/abs/2502.14847) |
| 中 | **`POST /workflows/socratic` — ソクラテス的対話ワークフロー (問題→仮説→反証→精緻化)** — `questioner` エージェントが前提・定義・論拠を問い続け (マイウティカ法)、`responder` エージェントが答えを精緻化し、最終的に `synthesizer` エージェントが対話ログから構造化された結論を抽出する。設計仕様の曖昧性解消・要件の深掘り・技術負債の根本原因分析などに適する。SocraSynth の段階的対立→協調モデルに従い、最初のラウンドは強い反論、後のラウンドは統合的問いに変化させる | SocraSynth (arXiv:2402.06634): モデレーター型のソクラテス的マルチエージェント討論プラットフォーム。KELE (EMNLP 2025 arXiv:2409.05511): LLM ベースのソクラテス教授エージェント実証。Towards Philosophical Reasoning (ChemRxiv 2025): 定義・類推・仮説消去の Socratic 技法を LLM エージェントに実装。[arxiv.org/abs/2402.06634](https://arxiv.org/abs/2402.06634) / [arxiv.org/html/2409.05511v1](https://arxiv.org/html/2409.05511v1) |
| 中 | **`/deliberate <question>` スラッシュコマンド — オーケストレートされたエージェントが自分自身の設計判断を内部討論させる** — 単一の親エージェントが `/deliberate "このAPIをREST vs GraphQLどちらにすべきか"` と入力すると、2 つのサブエージェント (advocate / critic) を自動スポーンし、2 ラウンドの討論後に `DELIBERATION.md` に結論を書き出して親に `deliberation_complete` STATUS を送信するコマンドを `.claude/commands/deliberate.md` として提供する | DEBATE (ACL 2024): 3エージェント構成の Devil's Advocate が単一 LLM 判断のバイアスを解消。CONSENSAGENT (ACL 2025): 迎合抑制プロンプトで効率的な合意形成を実証。既存の `/spawn-subagent` + `reply_to` + Workflow DAG の組み合わせで実現可能。[arxiv.org/abs/2405.09935](https://arxiv.org/abs/2405.09935) / [aclanthology.org/2025.findings-acl.1141](https://aclanthology.org/2025.findings-acl.1141) |
| 低 | **`heterogeneous_debate` エージェントグループ設定テンプレート — 異種エージェント討論のための `examples/debate_config.yaml`** — 同一モデルの複数インスタンスではなく、役割特化プロンプト (critic / advocate / judge) を持つ異種構成エージェントを `AgentConfig.groups` (v0.31.0) で `debate-pool` グループとして宣言し、`POST /workflows/debate` の `target_group` に渡せるサンプル設定ファイルを提供する。各エージェントの `system_prompt_file` に `.claude/prompts/roles/critic.md` 等を参照させることで、役割の固定化と迎合抑制を両立させる | A-HMAD (Springer 2025): 異種エージェント (論理推論 / 事実検証 / 戦略計画) の構成が均質構成より +10% 精度向上。ChatEval (ICLR 2024): 多様なペルソナが討論品質を決定する最重要因子。[link.springer.com/article/10.1007/s44443-025-00353-3](https://link.springer.com/article/10.1007/s44443-025-00353-3) / [arxiv.org/abs/2308.07201](https://arxiv.org/abs/2308.07201) |
| 低 | **`DECISION.md` 標準フォーマットと共有スクラッチパッドへの自動書き込み規約** — `debate` / `adr` / `delphi` / `redblue` / `socratic` の各ワークフローが出力を書き込む共通フォーマット (title / status / context / options_considered / decision / rationale / dissenting_opinions / consequences / references) を定義し、`GET /scratchpad/DECISION` で取得できるようにする。`context_files` に過去の `DECISION.md` を渡すことで次の討論に過去判断を参照させ、Codified Context (Vasilopoulos arXiv:2602.20478) の原則を意思決定ログにも適用する | SocraSynth (arXiv:2402.06634): 討論から「構造化された結論」を抽出する段階が必須。RT-AID (ScienceDirect 2025): 各ラウンドの中間出力が最終合意文書の品質を高める。既存の `GET/PUT /scratchpad/{key}` (v0.16.0) + `context_files` (v0.11.0) を組み合わせて実現可能。[arxiv.org/abs/2402.06634](https://arxiv.org/abs/2402.06634) / [arxiv.org/abs/2602.20478](https://arxiv.org/abs/2602.20478) |
