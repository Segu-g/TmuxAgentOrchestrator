...
| ~~中~~ | ~~**`/deliberate <question>` スラッシュコマンド** — エージェントが自発的に2エージェント討論を起動し `DELIBERATION.md` を生成~~ | ~~完了 v1.2.30 — `agent_plugin/commands/deliberate.md` 実装済み（v1.0.32）。v1.2.30 でロール別ガイダンス（worker/director/architect）追加、20テスト整備、研究記録（§10.105）追加。DEBATE arXiv:2405.09935 + SocraSynth arXiv:2402.06634 を設計根拠に採用。~~ | ~~層3~~ |
...
## §10.105 — v1.2.30 研究記録: `/deliberate` スラッシュコマンド文書化・ロール別ガイダンス整備

> 詳細記録: [docs/research/v1.2.30.md](docs/research/v1.2.30.md)

### 選択根拠

DESIGN.md §11「層3：ステージの実行方式」に `/deliberate <question>` スラッシュコマンドが優先度「中」として残っていた。`deliberate.md` コマンドは v1.0.32 で実装済みだが、v1.2.x 系列での研究記録・ロール別ドキュメント更新・テスト整備が欠如していた。

**選択しなかったもの**: トレースリプレイ CLI（依存インフラが複雑）、DECISION.md 標準フォーマット（ワークフロー間合意が必要）

### 調査結果サマリー

**1. 構造化討論による AI エージェントの意思決定改善**
- Du et al. "Devil's Advocate: Improving Multi-Agent Debate with Role Playing" arXiv:2405.09935 (ACL 2024): 2エージェント Devil's Advocate パターンが単一エージェント評価バイアスを大幅削減。強制的な対立ポジションが中立ポジションより強い論拠を生成。
- "Towards Human-AI Deliberation" CHI 2025 (dl.acm.org/doi/10.1145/3706598.3713423): LLM駆動 deliberative AI が過信と過小信頼を両方削減。
- CONSENSAGENT (ACL 2025): 明示的 critic ロールがスキャントを32%削減。

**2. ソクラテス的手法とソフトウェアアーキテクチャ意思決定**
- Chang "SocraSynth: Multi-LLM Reasoning with Conditional Statistics" arXiv:2402.06634 (Stanford, 2024): `deliberate.yaml` の questioner/responder/synthesizer トリオが直接対応。
- "MARS: Multi-Agent Framework Incorporating Socratic Guidance" arXiv:2503.16874 (2025): Teacher-Critic-Student 三者構造が反復的最適化に有効。

**3. スラッシュコマンド実装パターン**
- Claude Code Hooks Reference (Anthropic, 2025, code.claude.com/docs/en/hooks): `.claude/commands/<name>.md` + `$ARGUMENTS` パターンが標準。
- awesome-claude-code (GitHub 2025): 最も効果的なコマンドはマルチエージェント協調を実装。

### 実装サマリー

- `agent_plugin/commands/deliberate.md`: 実装済み（v1.0.32）— advocate/critic 2エージェント討論 + DELIBERATION.md 生成
- `agent_plugin/docs/worker.md`: `/deliberate` 使用ガイダンスセクション追加
- `agent_plugin/docs/director.md`: `/deliberate` 使用ガイダンスセクション追加（委任前に実施を推奨）
- `agent_plugin/docs/architect.md`: `/deliberate` 使用ガイダンスセクション追加（全 Key Design Decision 前に推奨）
- `docs/research/v1.2.30.md`: 本研究記録（詳細版）
- `tests/test_deliberate_command.py`: 20テスト全通過（構造検証 + mock HTTP サーバー統合テスト）
