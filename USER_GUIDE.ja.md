...
TmuxAgentOrchestrator は、tmux ペイン内で Claude Code エージェントのプールを実行し、優先度付きキューからタスクを配信するシステムです。各エージェントは、専用の tmux ペイン内で動作する `claude --dangerously-skip-permissions` プロセスで、キーボード入力とペイン出力のポーリングによって制御されます。
...
専用の tmux ペイン内で `claude --dangerously-skip-permissions` を駆動します。`CLAUDECODE` 環境変数は起動前に除去されるため、既存の Claude Code セッション内でも競合なく動作します。

**タスクの配信方法:** オーケストレーターは人間が入力するように `send_keys` を呼び出してペインにタスクプロンプトを送信します。

**起動シーケンス:** `claude` を起動後、最初の `❯` プロンプトが表示されて安定するまで（3 × 500 ms 変化なし）ペインをポーリングします。ワークスペース信頼ダイアログが表示された場合は自動的に承認します。その後にのみエージェントが IDLE としてマークされ、タスクを受け取れる状態になります。
...
```
start()  →  新規 tmux ペイン  →  コンテキストファイル書き込み  →  cd {worktree} && claude --dangerously-skip-permissions
         →  ❯ プロンプト待機  →  IDLE
...
- 同じ `session_name` のセッションが存在する場合は、強制終了して置き換える前に確認プロンプトが表示されます。`y` で続行、`n` で中止します。
...
- `claude_code.py` の `_DONE_PATTERNS` を確認し、使用している `claude` のバージョンのプロンプト文字に合うパターンを追加する。
...
### ペインに "Claude Code cannot be launched inside another Claude Code session" と表示される

`CLAUDECODE` 環境変数が設定された状態でオーケストレーターを実行しています。デフォルトのエージェントコマンドはこれを自動的に除去（`env -u CLAUDECODE claude …`）するため、デフォルト設定では発生しないはずです。設定でカスタム `command` を指定している場合は、先頭に `env -u CLAUDECODE` を追加してください。
