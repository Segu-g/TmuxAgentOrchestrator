...
TmuxAgentOrchestrator は、tmux ペイン内で AI エージェントのプールを実行し、優先度付きキューからタスクを配信するシステムです。各エージェントは次のいずれかです。

- **Claude Code エージェント** — 専用の tmux ペイン内で動作する `claude --no-pager` プロセス。キーボード入力とペイン出力のポーリングによって制御されます。
- **カスタムエージェント** — stdin/stdout 上の改行区切り JSON で通信する任意のスクリプト。
...
### ステップ 1 — tmux セッションを起動する

オーケストレーターには実行中の tmux サーバーが必要です。なければ起動してください:
...
agents:
  - id: worker-1
    type: claude_code   # `claude --no-pager` を駆動する
...
| `type` | `claude_code` \| `custom` | 必須 | 使用するエージェント実装 |
| `command` | 文字列 | `null` | `custom` エージェント用のシェルコマンド（必須）。`claude_code` では無視される |
...
```
start()  →  ペイン作成  →  コンテキストファイル書き込み  →  cd {worktree} && claude --no-pager
stop()   →  "q" を送信  →  ペイン監視解除  →  worktree 削除
```

**受信トレイ通知:** `claude_code` エージェントにメッセージが届くと、オーケストレーターはペインに `__MSG__:{msg_id}` と入力します。エージェントはその後 `/check-inbox` を使って読み取ることができます。

### `custom`

改行区切り JSON で通信するサブプロセスとして任意のスクリプトを実行します。

**プロトコル:**

```
stdin  ← {"task_id": "<uuid>", "prompt": "<タスクテキスト>"}
stdout → {"task_id": "<uuid>", "result": "<結果テキスト>"}
```

各行は完全な JSON オブジェクト 1 つです。スクリプトは各レスポンス後に stdout をフラッシュする必要があります。

**例 — `examples/echo_agent.py`:**
...
**作業ディレクトリ:** `isolate: true` の場合、サブプロセスの `cwd` はエージェントの git worktree に設定されます。`isolate: false` の場合、`cwd` はメインリポジトリのルートです。

**コンテキストファイル:** 起動時にエージェントは `__orchestrator_context__.json` を作業ディレクトリに書き込みます。これは `claude_code` エージェントと同じ形式で、REST API やメールボックスにコールバックする必要があるサブプロセスがオーケストレーターの座標を自動的に検出できるようにします。
...
`custom` エージェントでは、`_dispatch_task` は JSON 行を stdin に書き込んで即座に返ります。実際の処理は `_read_loop` で非同期に行われます。タイムアウトは*配信*をキャンセルするものであり、サブプロセスの計算をキャンセルするものではありません。スクリプトが任意の時間実行される可能性がある場合は、スクリプト内でタイムアウトを実装してください。
...
await bus.publish(Message(
    ...
    payload={
        ...
        "agent_type": "custom",
        "command": "python3 scripts/helper.py",
        "isolate": True,                 # 独自の worktree を付与（デフォルト）
        "share_parent_worktree": False,  # または True で親の cwd を再利用
    }
))
```
...
サブエージェントの ID は自動生成されます: `{parent_id}-sub-{6桁の16進数}`、例: `worker-1-sub-a3f2c1`。
...
ファイルを読み取るだけのエージェントや、隔離が不要なシンプルなスクリプトに使用してください。
...
| `/spawn-subagent` | `/spawn-subagent <type> [cmd]` | 自動 P2P 付きでサブエージェントを生成 |
...
- tmux が実行中か確認する: `tmux list-sessions`
- 設定のセッション名が既存のセッションに一致しているか確認する。または、オーケストレーターに作成させる。
