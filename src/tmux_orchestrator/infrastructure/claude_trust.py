...
def pre_trust_worktree(
    ...
    *,
    ...
    The write is **atomic**: the JSON is serialised to a temporary file in the
    same directory as ``~/.claude.json`` and then renamed into place using
    :func:`os.replace`.  This prevents a concurrent agent startup from reading
