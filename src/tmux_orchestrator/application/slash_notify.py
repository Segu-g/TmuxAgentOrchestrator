...
def _read_api_key(cwd: Path | None = None) -> str:
    ...
    File-based fallbacks (``__orchestrator_api_key__`` / per-agent variants)
    were removed because writing credentials to disk is an unnecessary
    security risk when the environment variable is always available.
