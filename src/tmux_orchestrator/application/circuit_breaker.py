...
Canonical location: tmux_orchestrator.application.circuit_breaker

Pure stdlib — no infrastructure dependencies.  The root-level shim
``circuit_breaker.py`` re-exports everything from here for backward compat.

Reference: Martin Fowler "Release It!" (2018) Ch. 5 — Stability Patterns.
Clean Architecture layer: application/ (pure in-memory state, no I/O).
